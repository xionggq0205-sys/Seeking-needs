...
export function extractJSON(text) {
  ...
  // 修复被截断的 JSON：从第一个 { 开始，按括号配对补全
  return repairJSON(t.slice(start));
}

// 尝试修复被 max_tokens 截断的 JSON。
// 策略：找出所有「结构安全切断点」（完成一个值后：闭引号/闭括号/数字/布尔/null 结尾），
// 从最靠后的切断点开始，截断 → 去尾逗号 → 补齐未闭合括号 → 尝试解析；失败则退到上一个切断点。
function repairJSON(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (c === '"') { inStr = !inStr; if (!inStr) safePoints.push(i); continue; }
    if (inStr) continue;
    if (c === "}" || c === "]") safePoints.push(i);
    else if (c >= "0" && c <= "9") safePoints.push(i);
    else if ((c === "e" && /tru|fals/.test(s.slice(i - 4, i))) || (c === "l" && s.slice(i - 3, i) === "nul")) safePoints.push(i);
  }

  // 从最靠后的安全点往前尝试，最多回退 60 次（足够丢掉一两个残缺字段）
  for (let p = safePoints.length - 1, tries = 0; p >= 0 && tries < 60; p--, tries++) {
    let body = s.slice(0, safePoints[p] + 1).replace(/,\s*$/, "");
    const close = [];
    ...
    try {
      const parsed = JSON.parse(body + close.reverse().join(""));
...
async function callOnce({ system, user, maxTokens, baseUrl, model, key }) {
  ...
  try {
    ...
    clearTimeout(timer);
    if (e.name === "AbortError") throw new Error("LLM 请求超时(35s)");
    throw e;
  }
  clearTimeout(timer);
...
export async function callLLM({ system, user, maxTokens = 2000 }) {
  ...
  try {
    return await callOnce({ system, user, maxTokens, baseUrl, model, key });
  } catch (primaryErr) {
    ...
    return await callOnce({ system, user, maxTokens, baseUrl: backupUrl, model: backupModel, key: backupKey });
  }
}
