...
// 关键原则:证据只能来自传入的真实条目(带真实 URL),不允许 Claude 编造。
...
export function buildQueries(expansion, fallback) {
  ...
  return uniq.length ? uniq.slice(0, 6) : [fallback];
}
...
// 把语料压缩成紧凑文本,控制 token
function corpusToText(items, cap = 36) {
  return items
    .slice(0, cap)
    .map((it, i) => {
      const body = `${it.title || ""} ${it.text || ""}`.replace(/\s+/g, " ").slice(0, 240);
...
export async function analyzeCorpus(input, items, { maxPains = 3, maxTokens = 3000 } = {}) {
  ...
  const text = await callClaude({ system: ANALYZE_SYSTEM(maxPains), user, maxTokens });
  const json = extractJSON(text);
