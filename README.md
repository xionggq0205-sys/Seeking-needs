...
api/analyze.js              Vercel Serverless:验证模式接口(扩词→抓数据→基于证据分析)
scripts/scan.js             发现模式扫描脚本(供 GitHub Actions / 本地运行)
lib/claude.js               Claude 调用封装 + JSON 提取
lib/sources.js              数据源适配器:Hacker News(免费) + Reddit(可选 OAuth)
lib/analyze.js              扩词 + 基于真实语料的痛点分析
...
> 只配 Anthropic 密钥 + Hacker News,就能完整跑起来。Reddit 是加分项,后补即可。

---

## 你需要准备的密钥

1. Anthropic API Key(必需):https://platform.deepseek.com 获取。
...
git init
git add .
git commit -m "init: DemandRadar v0.2"
git branch -M main
...
- 验证一次 = 两次 Claude 调用(扩词 + 分析)+ 若干次免费抓取,单次成本通常几美分量级。
- 发现扫描一次 = 一次较大的 Claude 分析 + 多次抓取,按周计成本很低。
